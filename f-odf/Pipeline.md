...
## Phase 1 — Dataset Build (`f-odf/build_dataset.py` → `preprocessing.py`)
...
### 1.6 Write Zarr
[`preprocessing.py:362`](../src/dw_thi/preprocessing.py) inside `build_fodf_dataset`
...
[`DWISliceDataset.__init__:82`](../src/dw_thi/f_odf/dataset.py)
...
- **`dti_scale`** [`dataset.py:205`](../src/dw_thi/f_odf/dataset.py): computed as `1 / percentile(abs(target_dti_6d), 99)` across all training subjects. Scales targets into O(1) range.
- **`max_bval`** [`dataset.py:202`](../src/dw_thi/f_odf/dataset.py): global maximum b-value across all subjects. Used to normalize bvals to [0, 1].
- **`canonical_hw`** [`dataset.py:198`](../src/dw_thi/f_odf/dataset.py): `(max_H, max_W)` across all slices and axes. All batches are padded to this.
- **`max_n`** [`dataset.py:176`](../src/dw_thi/f_odf/dataset.py): maximum number of DWI volumes across subjects. All batches are padded to this.
- **Sample index** [`dataset.py:184`](../src/dw_thi/f_odf/dataset.py): enumerates `(subject_key, axis, slice_index)` for every slice along every configured axis.
...
### 3.1 Load 2D slice
[`dataset.py:228`](../src/dw_thi/f_odf/dataset.py)

Two paths:
- **Worker RAM path** (fast): arrays preloaded by [`preload_dataset_in_worker:418`](../src/dw_thi/f_odf/dataset.py) via `worker_init_fn`. Array indexed directly in RAM.
- **Lazy zarr path**: [`_get_zarr_group:37`](../src/dw_thi/f_odf/dataset.py) opens a cached store handle per process; reads one 2D slice from disk.
...
Transpose to channel-first [`dataset.py:263`](../src/dw_thi/f_odf/dataset.py):
- `clean_nhw`: `(N, H, W)`
- `tgt_chw`: `(6, H, W)`

### 3.2 On-the-fly degradation
[`dataset.py:275`](../src/dw_thi/f_odf/dataset.py)
...
**GPU-deferred path** (CUDA only, `gpu_degrade=True`) [`dataset.py:285`](../src/dw_thi/f_odf/dataset.py):
...
**Eval mode** [`dataset.py:279`](../src/dw_thi/f_odf/dataset.py):
...
[`dataset.py:298`](../src/dw_thi/f_odf/dataset.py)
```
...
[`dataset.py:303`](../src/dw_thi/f_odf/dataset.py) (CPU path only; GPU path does this in `run_epoch`)
...
[`dataset.py:310`](../src/dw_thi/f_odf/dataset.py)
```
...
[`dataset.py:317`](../src/dw_thi/f_odf/dataset.py) — only when `augment=True`

**Flip** [`dataset.py:320`](../src/dw_thi/f_odf/dataset.py) — `AUG_FLIP=True`:
...
- Also applies:
  - [`_flip_dti6d_sign:55`](../src/dw_thi/f_odf/dataset.py): negates off-diagonal tensor components that touch the flipped axis (e.g., x-flip negates Dxy and Dxz at channels 1 and 3)
  - [`_flip_bvecs:63`](../src/dw_thi/f_odf/dataset.py): negates the corresponding bvec component

**Intensity jitter** [`dataset.py:341`](../src/dw_thi/f_odf/dataset.py) — `AUG_INTENSITY=0.1`:
```
scale ~ Uniform(0.9, 1.1)
input_nhw = input_nhw * scale
```

**Volume dropout** [`dataset.py:346`](../src/dw_thi/f_odf/dataset.py) — `AUG_VOLUME_DROPOUT=0.1`:
- Each volume independently zeroed with probability 0.1
- Zeroed volumes also set to 0 in `vol_mask`

### 3.7 Padding to batch shape
[`dataset.py:360`](../src/dw_thi/f_odf/dataset.py)
...
**Spatial dimensions** [`dataset.py:371`](../src/dw_thi/f_odf/dataset.py): zero-pad `input_nhw`, `tgt_chw`, `bmask_hw` from `(H, W)` to `canonical_hw`.

### 3.8 Output tensors
[`dataset.py:381`](../src/dw_thi/f_odf/dataset.py)
...
[`train.py:532`](../src/dw_thi/f_odf/train.py)
...
[`train.py:542`](../src/dw_thi/f_odf/train.py): `pred = model(signal, bvals, bvecs, vol_mask)`

### 5.1 Q-space encoder
[`model.py:75` `QSpaceEncoder.forward`](../src/dw_thi/f_odf/model.py)
...
### 5.2 U-Net
[`model.py:159` `UNet2D.forward`](../src/dw_thi/f_odf/model.py)
...
Each `ConvBlock` [`model.py:106`](../src/dw_thi/f_odf/model.py): `Conv2d → GroupNorm(8) → SiLU → Dropout2d → Conv2d → GroupNorm(8) → SiLU`

### 5.3 Cholesky output (when `cholesky=True`)
[`model.py:14` `cholesky_to_tensor6`](../src/dw_thi/f_odf/model.py)
...
[`train.py:544`](../src/dw_thi/f_odf/train.py): `loss, metrics = criterion(pred.float(), target, mask=brain_mask)`

[`loss.py:81` `DTILoss.forward`](../src/dw_thi/f_odf/loss.py)

All terms are masked: only brain voxels contribute.

**Tensor term** (always active):
- `residual = pred - target` (both in dti_scale space)
- [`_charbonnier:39`](../src/dw_thi/f_odf/loss.py): `sqrt(residual² + eps²)` with `eps=1e-3` — behaves like MSE near zero, L1 for large residuals
- Normalized by `sum(brain_mask) * 6`

**Scalar terms** (`LAMBDA_SCALAR=0.3`):
- [`tensor6_to_fa_md:12`](../src/dw_thi/f_odf/loss.py): computes FA and MD analytically from 6D tensor using Frobenius norms (no eigendecomposition)
- `FA MAE` + `MD MAE` masked to brain voxels

**Edge term** (`LAMBDA_EDGE=0.1`):
- [`_spatial_grad_mag:48`](../src/dw_thi/f_odf/loss.py): `|Δx FA| + |Δy FA|` for pred and target
...
[`train.py:551`](../src/dw_thi/f_odf/train.py)
...
**LR schedule** [`train.py:874`](../src/dw_thi/f_odf/train.py):
