...
class FodfLoss(nn.Module):
    ...
    def _sample_sf_at_indices(
        ...
        basis = basis_by_direction[indices]
        return (sh_hw.unsqueeze(1) * basis).sum(dim=-1)
    ...
    def _surface_fodf_terms(
        ...
        peak_weight = valid_peak.to(dtype=pred_fodf.dtype)
        peak_den = peak_weight.sum().clamp(min=1)
        peak_residual = pred_peak_values - top_values
        peak_loss = (
            _charbonnier(peak_residual, self.charbonnier_eps) * peak_weight
        ).sum() / peak_den
        peak_mae = (peak_residual.abs() * peak_weight).sum() / peak_den
        peak_ratio = (
            (pred_peak_values.clamp_min(0) / top_values.clamp_min(1e-6)) * peak_weight
        ).sum() / peak_den
    ...
    def _anisotropic_corr_loss(
        ...
        weight = valid.to(dtype=pred_fodf.dtype)
        valid_count = weight.sum()
        cosine = F.cosine_similarity(pred_vox, tgt_vox, dim=1, eps=1e-8)
        cosine_mean = (cosine * weight).sum() / valid_count.clamp(min=1)
        has_valid = (valid_count > 0).to(dtype=pred_fodf.dtype)
        return (1.0 - cosine_mean) * has_valid
    ...
    def forward(
        ...
        valid_weight = valid.to(dtype=pred_fodf.dtype)
        valid_count = valid_weight.sum()
        cosine = F.cosine_similarity(pred_vox, tgt_vox, dim=1, eps=1e-8)
        fodf_acc = (cosine * valid_weight).sum() / valid_count.clamp(min=1)
        has_valid = (valid_count > 0).to(dtype=pred_fodf.dtype)
        fodf_acc = fodf_acc * has_valid
        fodf_corr_loss = (1.0 - fodf_acc) * has_valid
