...
class DirectionConditionedResidualDenoiser(nn.Module):
    ...
    The same small CNN is shared across volumes. Direction metadata modulates
    each residual block through FiLM, so the network can learn b-value and
    direction-dependent corruption patterns without allocating a huge
    ``N``-channel image model.
    """
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList(
            [FiLMResidualBlock(channels, dropout=dropout, film_dim=grad_hidden) for _ in range(depth)]
        )
    ...
    def forward(
        ...
        denoised = signal - residual
        denoised = denoised * vol_mask[:, :, None, None]
...
class MRDCNN(nn.Module):
    """Model Residual Denoising CNN for DWI -> 6D DTI prediction.

    MRD-CNN first predicts the residual corruption component in DWI space,
    subtracts that correction from the input signal, then estimates the DTI
    tensor with a lightweight residual CNN. It preserves the same public model
    contract as QSpaceUNet and QSpaceAttentionNet.
    """
