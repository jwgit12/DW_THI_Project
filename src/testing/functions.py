...
def find_dwi_datasets(root_dir):
    ...
    # explicitly target BIDS DWI folders
    pattern = os.path.join(root_dir, "sub-*", "ses-*", "dwi", "*_dwi.nii.gz")
    dwi_files = glob.glob(pattern)
    ...
    for dwi_path in dwi_files:
        ...
        if os.path.exists(bval_path) and os.path.exists(bvec_path):
            ...
            print(f"[WARN] Missing gradients for {dwi_path}")

    print(f"\n[INFO] Found {len(datasets)} valid DWI datasets\n")
