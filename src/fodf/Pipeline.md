...
## Phase 1 — Dataset Build (`build_fodf_dataset.py` → `preprocessing.py`)

Run once. Stores only clean data.

### 1.1 Discover files
[`preprocessing.py:65` `find_dwi_datasets`](../dw_thi/preprocessing.py)
- Globs `*_dwi.nii.gz` under `--data_dir`
- Checks for matching `.bval` / `.bvec` sidecars
- Parses BIDS entities (sub, ses, run) → unique Zarr group key via [`parse_dwi_entities:34`](../dw_thi/preprocessing.py)

### 1.2 Load raw DWI
[`preprocessing.py:90` `load_dwi_dataset`](../dw_thi/preprocessing.py)
...
### 1.3 Brain mask
[`preprocessing.py:153` `compute_brain_mask_from_dwi`](../dw_thi/preprocessing.py)
- [`mean_b0_volume:141`](../dw_thi/preprocessing.py): averages all volumes with bval < 50 → `(X, Y, Z)` reference
- `dipy.segment.mask.median_otsu` with `median_radius=4`, `numpass=4`, `dilate=1`, `finalize_mask=True`
- Output: `(X, Y, Z)` bool mask

### 1.4 DTI fit
[`preprocessing.py:112` `compute_dti`](../dw_thi/preprocessing.py)
...
### 1.5 Flatten to 6D
[`preprocessing.py:119` `tensor_to_6d`](../dw_thi/preprocessing.py)
- Extracts 6 unique components: **[Dxx, Dxy, Dyy, Dxz, Dyz, Dzz]**
- Output: `(X, Y, Z, 6)` float32

### 1.6 Write Zarr
[`preprocessing.py:362`](../dw_thi/preprocessing.py) inside `build_fodf_dataset`
...
[`DWISliceDataset.__init__:82`](dataset.py)
...
- **`dti_scale`** [`dataset.py:205`](dataset.py): computed as `1 / percentile(abs(target_dti_6d), 99)` across all training subjects. Scales targets into O(1) range.
- **`max_bval`** [`dataset.py:202`](dataset.py): global maximum b-value across all subjects. Used to normalize bvals to [0, 1].
- **`canonical_hw`** [`dataset.py:198`](dataset.py): `(max_H, max_W)` across all slices and axes. All batches are padded to this.
- **`max_n`** [`dataset.py:176`](dataset.py): maximum number of DWI volumes across subjects. All batches are padded to this.
- **Sample index** [`dataset.py:184`](dataset.py): enumerates `(subject_key, axis, slice_index)` for every slice along every configured axis.
...
### 3.1 Load 2D slice
[`dataset.py:228`](dataset.py)

Two paths:
- **Worker RAM path** (fast): arrays preloaded by [`preload_dataset_in_worker:418`](dataset.py) via `worker_init_fn`. Array indexed directly in RAM.
- **Lazy zarr path**: [`_get_zarr_group:37`](dataset.py) opens a cached store handle per process; reads one 2D slice from disk.
...
Transpose to channel-first [`dataset.py:263`](dataset.py):
- `clean_nhw`: `(N, H, W)`
- `tgt_chw`: `(6, H, W)`

### 3.2 On-the-fly degradation
[`dataset.py:275`](dataset.py)
...
- Call [`augment.py:73` `degrade_dwi_slice(clean_nhw, kf, nl, rng)`](../dw_thi/augment.py):
  1. [`lowres_kspace_cutout:34`](../dw_thi/augment.py): `scipy.fft.rfft2` on `(N, H, W)` → zero out rows `[ry : H-ry]` and columns `[rx :]` where `ry = int(H * kf / 2)`, `rx = int(W * kf / 2)` → `scipy.fft.irfft2` back to image space
  2. [`add_scaled_gaussian_noise:57`](../dw_thi/augment.py): `sigma = nl * max(slice)` per volume → add `N(0, sigma)` noise

**GPU-deferred path** (CUDA only, `gpu_degrade=True`) [`dataset.py:285`](dataset.py):
- `clean_nhw` is returned unchanged; `degrade_kf`, `degrade_nl`, `b0_mask` added to the batch dict
- Actual degradation runs in `run_epoch` after the batch is on GPU via [`augment.py:120` `gpu_degrade_dwi_batch`](../dw_thi/augment.py)

**Eval mode** [`dataset.py:279`](dataset.py):
...
[`dataset.py:303`](dataset.py) (CPU path only; GPU path does this in `run_epoch`)
- Identifies b0 volumes: `bvals < B0_THRESHOLD=50`
- `mean_b0 = mean over b0 volumes` → `(H, W)`
- [`preprocessing.py:134` `compute_b0_norm`](../dw_thi/preprocessing.py): `brain_voxels = mean_b0[mean_b0 > 0.1 * max]` → `norm = mean(brain_voxels)`
- `input_nhw = input_nhw / norm`

### 3.5 bval normalization
[`dataset.py:310`](dataset.py)
```
...
[`dataset.py:317`](dataset.py) — only when `augment=True`

**Flip** [`dataset.py:320`](dataset.py) — `AUG_FLIP=True`:
...
- Also applies:
  - [`_flip_dti6d_sign:55`](dataset.py): negates off-diagonal tensor components that touch the flipped axis (e.g., x-flip negates Dxy and Dxz at channels 1 and 3)
  - [`_flip_bvecs:63`](dataset.py): negates the corresponding bvec component

**Intensity jitter** [`dataset.py:341`](dataset.py) — `AUG_INTENSITY=0.1`:
```
scale ~ Uniform(0.9, 1.1)
input_nhw = input_nhw * scale
```

**Volume dropout** [`dataset.py:346`](dataset.py) — `AUG_VOLUME_DROPOUT=0.1`:
- Each volume independently zeroed with probability 0.1
- Zeroed volumes also set to 0 in `vol_mask`

### 3.7 Padding to batch shape
[`dataset.py:360`](dataset.py)
...
**Spatial dimensions** [`dataset.py:371`](dataset.py): zero-pad `input_nhw`, `tgt_chw`, `bmask_hw` from `(H, W)` to `canonical_hw`.

### 3.8 Output tensors
[`dataset.py:381`](dataset.py)
...
[`train.py:532`](train.py)

When `degrade_kf` is present in the batch (GPU-degrade mode):

1. [`augment.py:120` `gpu_degrade_dwi_batch(signal, kf, nl)`](../dw_thi/augment.py):
...
2. [`augment.py:166` `gpu_b0_normalize_batch(signal, b0_mask)`](../dw_thi/augment.py):
...
[`train.py:542`](train.py): `pred = model(signal, bvals, bvecs, vol_mask)`

### 5.1 Q-space encoder
[`model.py:75` `QSpaceEncoder.forward`](model.py)
...
### 5.2 U-Net
[`model.py:159` `UNet2D.forward`](model.py)
...
Each `ConvBlock` [`model.py:106`](model.py): `Conv2d → GroupNorm(8) → SiLU → Dropout2d → Conv2d → GroupNorm(8) → SiLU`

### 5.3 Cholesky output (when `cholesky=True`)
[`model.py:14` `cholesky_to_tensor6`](model.py)
...
[`train.py:544`](train.py): `loss, metrics = criterion(pred.float(), target, mask=brain_mask)`

[`loss.py:81` `DTILoss.forward`](loss.py)

All terms are masked: only brain voxels contribute.

**Tensor term** (always active):
- `residual = pred - target` (both in dti_scale space)
- [`_charbonnier:39`](loss.py): `sqrt(residual² + eps²)` with `eps=1e-3` — behaves like MSE near zero, L1 for large residuals
- Normalized by `sum(brain_mask) * 6`

**Scalar terms** (`LAMBDA_SCALAR=0.3`):
- [`tensor6_to_fa_md:12`](loss.py): computes FA and MD analytically from 6D tensor using Frobenius norms (no eigendecomposition)
- `FA MAE` + `MD MAE` masked to brain voxels

**Edge term** (`LAMBDA_EDGE=0.1`):
- [`_spatial_grad_mag:48`](loss.py): `|Δx FA| + |Δy FA|` for pred and target
...
**LR schedule** [`train.py:874`](train.py):
